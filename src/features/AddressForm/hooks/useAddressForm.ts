...
export function useAddressForm(): UseAddressFormReturn {
    ...
    const lookupLocality = async () => {
      try {
        ...
        if (data.length === 1) {
          ...
          setPostalOptions([])
          setError("Keine Ergebnisse für diesen Ort gefunden")
        }
      } catch (err: any) {
        if (err.name !== "AbortError") setError("Ort konnte nicht abgerufen werden")
      } finally {
        setLoading(false)
      }
    }
  ...
  useEffect(() => {
    ...
    if (!debouncedPostal) return

    if (debouncedPostal.length !== 5) {
      setError("Die Postleitzahl muss genau 5-stellig sein")
      return
    }
    ...
    const lookupPostal = async () => {
      try {
        ...
        if (data.length === 0) {
          setError("Invalid postal code — not found in Germany")
          setLocality("")
        } else {
